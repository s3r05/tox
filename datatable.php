...
$aColumns = array( 's.id','s.sname','s.balance','s.fees','b.grade','s.contact','s.joindate');
	...
	while ( $aRow = $rResult->fetch_assoc()  )
	{
		...
		$row = array(
					...
					html_entity_decode('<button class="btn btn-success btn-sm" style="border-radius:0%" onclick="javascript:GetFeeForm('.$aRow['id'].')"><i class="fa fa-money"></i>  دانا قستی </button>')
...
$aColumns = array( 's.id','s.sname','s.balance','s.fees','b.grade','s.contact','s.joindate');
	...
	while ( $aRow = $rResult->fetch_assoc()  )
	{
		...
		$row = array(
					...
					html_entity_decode('<button class="btn btn-success btn-sm" style="border-radius:0%" onclick="javascript:GetFeeForm('.$aRow['id'].')">  راپۆرت </button>')
